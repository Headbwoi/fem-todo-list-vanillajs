...
const showTodo = () => {
  ...
  if (todos) {
    todos.forEach((todo, index) => {
      ...
      let isCompleted = todos[index].status == "completed" ? "checked" : ""

      li += `
          <li
              class="flex items-center justify-start h-[3.25rem] md:h-[4.0625rem] pl-6 pr-5 border-b border-Light_Grayish_Blue_light dark:border-b dark:border-Very_Dark_Grayish_Blue_dark1 relative overflow-hidden group cursor-pointer"
            >
              <label for="${index}">
                <input type="checkbox" id="${index}" class="todo hidden" ${isCompleted}/>
                <span
...
showTodo()
// looping through todo items and adding active class and passing clicked to updateStatus function
...
userInput.addEventListener("keyup", (e) => {
  ...
  if (e.key == "Enter" && userInputTask) {
    ...
    let userTask = { name: userInputTask, status: "pending" } //creates todo object
  ...
  showTodo() //calls showTodo function
})
